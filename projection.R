...
data <- read.csv('~/Desktop/GAM_final/GAM_monthly_dataframes/monthly_GAM_all_groups.csv')
est_us <- read.csv("~/Desktop/GAM_final/GAM_monthly_dataframes/est_us_new_updated.csv")
...
Tus <- read.csv('~/Downloads/NCC2018-master/inputs/projections/TemperatureProjections_us.csv') %>% as_data_frame()

Tus$deltaT <- Tus$deltaT 
...
## 2020 to 2100
p1 <- pp[pp[,3]=="United States of America",paste0(2020:2050)]
...
dUS <- as.matrix(est_us)%*%matrix(Tus$deltaT,nrow=1)*12  #additional deaths per 100,000 per year by 2100 in US
...
#Panel (b)
plot(yr,cumsum(ex[1,1,])/1000,type="l",ylim=c(-8,ub),col=cll,las=1,ylab="excess suicides (thousands)",xlab="year",lwd=0.3,xlim=c(2020,2052),cex.axis=1.5,cex.lab=1.5,axes=F)
for (j in 1:dim(ex)[1]) {
  for (i in 1:dim(ex)[2]) {
...
axis(1,at=seq(2020,2050,10),labels=seq(2020,2050,10),cex.axis=1.5,las=1)
axis(2,at=seq(0,ub,10),labels=seq(0,ub,10),cex.axis=1.5,las=1)
